...
void sigxcputest1(){
	struct procent *prptr = &proctab[currpid];
	if(registercbsig(MYSIGXCPU,&sigxcpuhandler1,875) != OK) {
...
int memalloc(){
	...
	//kprintf("The address is %08X\n",&prptr->prmemblklist);
	//kprintf("The prptr->memblklist->next is %08X\n",&prptr->prmemblklist->next);
	return 0;
}
...
int memdealloc(){
	...
	printf("Testing : allocated %d\r\n",prptr->prname,prptr->prmemblklist->size);
...
process main(void)
{
	...
	//pid32 garbage = create((void*)gctest,2048,20,"gctest",0,NULL);
	//resume(garbage);
	//resume(create((void *)memdealloc,2048,20,"memd",0,NULL));

	//sleepms(3000);

	//printf(" memory test : xinu has free memory %d\r\n",proctab[currpid].memlength);


	resume(create((void *)memalloc,2048,20,"mem1",0,NULL));
	sleepms(2000);
	//kprintf("AY!");
